...
for input in inputArray {
    ...
    let action = String(inputs[0])
    let word = String(inputs[1])

    switch action {
    case Action.add.rawValue:
        trie.insert(word: word)
    case Action.find.rawValue:
        if let trie = trie.findLastNodeOf(word: word) {
            print("\(trie.size)")
            break
        }
        print("\(0)")
